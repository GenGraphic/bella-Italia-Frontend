...
const Kasse = ({closeThis, toPay, steuer, netto, versand}) => {
    ...
    const total = (Math.round(toPay * 100) / 100).toFixed(2);
    const countriesList = [
        "Österreich",
        "Belgien",
        "Bulgarien",
    ...
    useEffect(() => {
        ...
        if (orderApproved) {
            handleThankYou();
            setOrderApproved(false); // Reset for future transactions
        }
    }, [orderApproved])
        ...
        <form>
            <div className='d-flex justify-content-between my-3 flex-column gap-1'>
                ...
                <div className='d-flex gap-2'>
                   ...
                   <input type='text' placeholder='Musterman' name='name' value={formData.name} onChange={handleChangeInput}/>  
                </div>
            </div>
            <div className='d-flex justify-content-between my-3 flex-column gap-1'>
                <label>Email: *</label>
